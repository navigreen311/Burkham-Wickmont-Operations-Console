...
31 specs across two harnesses - 13 for the portal, 18 for the internal Console, each with its own
`webServer` on its own port because they are two processes on two trust boundaries (ADR-0022).
`pnpm test:e2e` runs **31 in Chromium**; `E2E_ALL_ENGINES=1 pnpm test:e2e` adds the cross-engine
