...
6. **State compliance is a workflow gate.** The Regulatory Engine runs *before* client-facing
...
- **`approvedCreditLimit`, never `creditLimit`, for money owed to us.** `creditLimit` is what was
  *requested*; `approvedCreditLimit` is what was *granted*. Success fees compute against
...
- **Authority Level 4 actions are hard-blocked by middleware and logged** — sign for a client,
  ...
  Success criterion: *zero Level 4 actions succeed*; blocked-and-logged is the only outcome.
